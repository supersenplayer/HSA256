...
begin
    ...
    -- working registers + H accumulator
    process(clk)
    begin
        if rst = '1' then
            ...
            -- reinitialize working registers at the start of each new hash
            if hash_init = '1' then
                ra <= H_init(0); rb <= H_init(1);
                rc <= H_init(2); rd <= H_init(3);
                re <= H_init(4); rf <= H_init(5);
            ...
            -- register the round outputs each active cycle
            ...
            -- accumulate final hash: fires exactly 1 cycle after round 63
            -- at that point ra..rh hold the final working variable values
            if compute_done = '1' then
                H_result(0) <= add(H_init(0), ra);
                H_result(1) <= add(H_init(1), rb);
                H_result(2) <= add(H_init(2), rc);
                H_result(3) <= add(H_init(3), rd);
                H_result(4) <= add(H_init(4), re);
                H_result(5) <= add(H_init(5), rf);
                H_result(6) <= add(H_init(6), rg);
                H_result(7) <= add(H_init(7), rh);
            end if;
